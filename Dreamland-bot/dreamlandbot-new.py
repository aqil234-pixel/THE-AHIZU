...
async def chat_ai(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Bikin AI-nya sadar peran
    system_prompt = (
        "Kamu adalah asisten admin toko ikan hias 'Dreamlandfish.myd'. "
        "Jawab pertanyaan pelanggan dengan seru, tidak kaku , asik, dan singkat, to the point. (seperti manusia )"
    ...
    try:
        # Nembak API Groq (Gue setting pake model LLaMA 3 yang kenceng)
        chat_completion = groq_client.chat.completions.create(
            messages=[
                {"role": "system", "content": system_prompt},
                {"role": "user", "content": pesan_user}
            ],
            model="llama-3.1-8b-instant",  # <--- GANTI JADI INI
        )
        ...
        await update.message.reply_text("Waduh, admin lagi sibuk bungkus ikan nih. Coba chat lagi bentar ya! 😅")
